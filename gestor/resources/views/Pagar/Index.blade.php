...
<section class="caixa">
	<div class="thead border-bottom mb-3 p-1">
		<div class="titulo mb-0">
			...
			<span class="mig"><a href="{{route('index')}}" class="text-azul"> Home </a> <i class="fas fa-angle-double-right text-azul"></i> <b class="text-azul2">Lista de contas a pagar</b></span>
		</div>
		<div class="text-end d-flex">
			<a href="{{route('pagar.create')}}" class="btn btn-azul d-inline-block mx-1" title="Adicionar Novo"><i class="fas fa fa-plus-circle" aria-hidden="true"></i> </a>
		</div>
	</div>
						...
						@foreach($lista as $v)
							<tr>
                                ...
                                <td align="center"><span class="{{ strtolower($v->status->status) }}">{{ $v->status->status }}</span></td>
									...
									<td  class="text-center"><a href="{{route('pagar.faturar', $v->id)}}" class="d-inline-block btn btn-roxo btn-pequeno" title="Faturar"><i class="fas fa-donate"></i> </a>  
									<a href="{{route('pagar.edit', $v->id)}}" class="d-inline-block btn btn-verde btn-pequeno btn-circulo" title="Editar"><i class="fas fa-edit"></i></a> 								
                                    ...
                                    <a href="#" onclick="confirm('Tem Certeza?') ? document.getElementById('apagar{{$v->id}}').submit() : '';" class="d-inline-block btn btn-vermelho btn-circulo btn-pequeno" title="Excluir"><i class="fas fa-trash-alt"></i></a>
                                        <form action="{{route('pagar.destroy', $v->id)}}" method="POST" id="apagar{{$v->id}}">
...
<script>
	function pesquisar(mes){
		var ano = $("#ano").val();
		window.location.href=base_url + "pagar/pormes/?ano=" + ano + "&mes=" + mes;
	}
</script>
@endsection