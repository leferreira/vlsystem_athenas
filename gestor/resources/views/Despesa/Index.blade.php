...
<section class="caixa">
	<div class="thead border-bottom mb-3 p-1">
		<div class="titulo mb-0">
			<span><i class="fas fa-list-alt"></i> Lista de Despesas</span>
			<span class="mig"><a href="{{route('index')}}" class="text-azul"> Home </a> <i class="fas fa-angle-double-right text-azul"></i> <b class="text-azul2">Lista de contas a despesa</b></span>
		</div>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" id="dataTable">
						...
						@foreach($lista as $v)
							<tr>
								...
								<td  class="text-center">{{$v->fornecedor->razao_social}}</td>
                                ...
                                <td align="center"><span class="{{ strtolower($v->status->status) }}">{{ $v->status->status }}</span></td>
									...
									<td  class="text-center"><a href="{{route('despesa.faturar', $v->id)}}" class="d-inline-block btn btn-roxo btn-pequeno" title="Faturar"><i class="fas fa-donate"></i> </a>  
									<a href="{{route('despesa.edit', $v->id)}}" class="d-inline-block btn btn-verde btn-pequeno btn-circulo" title="Editar"><i class="fas fa-edit"></i></a> 								
    								
                                    <a href="#" onclick="confirm('Tem Certeza?') ? document.getElementById('adespesa{{$v->id}}').submit() : '';" class="d-inline-block btn btn-vermelho btn-circulo btn-pequeno"><i class="fas fa-trash-alt"></i></a>
                                        <form action="{{route('despesa.destroy', $v->id)}}" method="POST" id="adespesa{{$v->id}}">
...
<script>
	function pesquisar(mes){
		var ano = $("#ano").val();
		window.location.href=base_url + "despesa/pormes/?ano=" + ano + "&mes=" + mes;
	}
</script>
@endsection