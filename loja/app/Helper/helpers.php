...
function calculaFrete($valores){
    $parametros = [
        "nCdEmpresa"            => "",
        "sDsSenha"              =>"",
        "nCdServico"            => $valores->codigoServico ?? 0,
        "sCepOrigem"            => $valores->cepOrigem  ?? 0,
        "sCepDestino"           => $valores->cepDestino  ?? 0,
        "nVlPeso"               => $valores->peso  ?? 0,
        "nCdFormato"            => $valores->formato  ?? 0,
        "nVlComprimento"        => $valores->comprimento  ?? 0,
        "nVlAltura"             => $valores->altura  ?? 0,
        "nVlLargura"            => $valores->largura  ?? 0,
        "nVlDiametro"           => $valores->diametro  ?? 0,
        "sCdMaoPropria"         => $valores->maoPropria  ?? "N",
        "nVlValorDeclarado"     => $valores->valorDeclarado  ?? "N",
        "sCdAvisoRecebimento"   => $valores->avisoRecebimento  ?? "N",
        "StrRetorno"            =>"xml"
    ];
    ...
    $retorno    = enviarGetCurlSDecode($url);
    $resultado  = strlen($retorno) ? simplexml_load_string($retorno) : null;
    return $resultado->cServico ?? null;
}
