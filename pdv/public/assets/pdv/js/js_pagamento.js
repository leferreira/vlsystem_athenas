...
$(document).ready(function() {
	...
	$('.tecla').bind('keydown', 'Ctrl+2',function() {
		abrirModal("#telaArmazendarVenda");	
		return false;
	});
...
function gerarCrediario(){
	var valor_pagamento_parcela 	= converteMoedaFloat($("#valor_pagamento_parcela").val());
	var limite_cliente 				= converteMoedaFloat($("#limite_cliente").val());
	var data_pagamento_parcela 		= $("#data_pagamento_parcela").val();	
	var forma_de_parcelar_crediario = $("#forma_de_parcelar_crediario").val();
	var qtde_parcela_crediario 		= $("#qtde_parcela_crediario").val();	

	if(limite_cliente < valor_pagamento_parcela){
		alert("Este valor é inferior ao limite disponível");
		return false;
	}
	if(valor_pagamento_parcela > TOTAL_RESTANTE){
		alert("Valor superior ao total restante");
		return false;
	}
	$.ajax({
		type: 'POST',
		data: {
			...
			valor: valor_pagamento_parcela,
			data_pagamento_parcela: data_pagamento_parcela,
			forma_de_parcelar_crediario: forma_de_parcelar_crediario,
			qtde_parcela_crediario: qtde_parcela_crediario,	
		},
		url: base_url + 'pdv/gerarCrediario',
		dataType: 'json',
		beforeSend: function (){
		   giraGira();
	   },
...
function gerarPagamentoCartao(){
	var valor_pagamento_cartao 		= converteMoedaFloat($("#valor_pagamento_cartao").val());
	var data_pagamento_cartao 		= $("#data_pagamento_cartao").val();	
	var bandeira_id 				= $("#bandeira_id").val();
	var qtde_parcela_cartao 		= $("#qtde_parcela_cartao").val();	
	
	if(valor_pagamento_cartao > TOTAL_RESTANTE){
		alert("Valor superior ao total restante");
		return false;
	}
	$.ajax({
		...
		data: {
			venda_id: $("#venda_id").val(),
			data_pagamento_cartao: data_pagamento_cartao,
			valor: valor_pagamento_cartao,
			bandeira_id: bandeira_id,
			qtde_parcela_cartao: qtde_parcela_cartao,	
		},
		url: base_url + 'pdv/gerarPagamentoCartao',
		dataType: 'json',
		beforeSend: function (){
		   giraGira();
	   },
...
function selecionarFormaPagamento(id_forma,txt_forma){
	...
	if(id_forma==5){
		if(cliente_id==null | cliente_id ==""){
			alert("Adicione primeiramente um cliente à venda! ");
			return false;
		}
		
		if(credito_liberado != "S"){
			alert("O crédito deste cliente não está liberado! ");
			return false;
		}
		abrirModalCrediario();
		return false;		
	...
	marcarPagamentoSelecionado(id_forma);
	if(id_forma==5){		
		return false;
	}
...
function abrirModalCrediario(){
	$("#valor_pagamento_parcela").val(converteFloatMoeda(TOTAL_RESTANTE));
	abrirModal("#verTelaFormaPagtoCrediario");
}


function abrirModalCartaoCredito(){
	$("#valor_pagamento_cartao").val(converteFloatMoeda(TOTAL_RESTANTE));
	abrirModal("#verTelaFormaPagtoCartaoCredito");
}
...
function calculaTroco(){
	...
	var valor_troco 	=  valor_pago - TOTAL_RESTANTE;
...
function salvarVenda() {	
	...
	let js = {
		...
		venda_id		: $('#venda_id').val(),	
		tem_pendencia	: $('#tem_pendencia').val(),	
	}
	$.ajax({
		type: 'POST',
		data: {
...
function cancelarVenda(){
	...
	if (confirm("Tem certeza que deseja Cancelar esta venda ") == true) {
		$.ajax({
			...
			url: base_url + 'venda/cancelarVenda',
			dataType: 'json',
			beforeSend: function (){
			   giraGira();
		   },
...
function marcarPagamentoSelecionado(id_forma){
	$(".frmPagto").removeClass("selecionado");
	$("#frm"+id_forma).addClass("selecionado");
}
