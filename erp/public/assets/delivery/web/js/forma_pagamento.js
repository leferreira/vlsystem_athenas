	...
	getDadosCalculoEntrega((res) => {
		...
		getCurrentLocation((crd) => {
			...
			if(crd){
				...
				alert("Atenção!", 'Não foi possivel recuperar sua localização, ative e recarregue a pagina!', "warning")
...
$('#pagseguro').click(() => {
	...
	if(telefone.length > 12 && enderecoSelecionado != null){
		...
		if(!enderecoSelecionado){
			...
			alert("Atenção!", "Por favor selecione a forma de entrega!", "warning")

		}
		else if(telefone.length < 12){

			alert("Atenção!", "Por favor informe um telefone de contato (11) 99999-9999", "warning")
...
$('#salvar_endereco').click(() => {
	...
	$.post(path+'enderecodelivery', {_token : tk, data: js})
	.done(function(data){
...
$('#finalizar-venda').click(() => {
	...
	if(!formaPagamento){
		alert("Atenção! - Por favor selecione a forma de pagamento!")
	}
	else if(!enderecoSelecionado){
		alert("Atenção! - Por favor selecione a forma de entrega!")
	}
	else if(telefone.length <= 12){
		alert("Atenção! - Por favor informe um telefone de contato (11) 99999-9999")

	}

	else if(formaPagamento == 'dinheiro' && troco.length == 0 || parseFloat(troco.replace(",", ".")) == 0){

		alert("Atenção! - Por favor insira o valor de troco para!")
	}

	else if(formaPagamento == 'dinheiro' && parseFloat(troco.replace(",", ".")) < TOTAL){
		alert("Atenção! - Valor do troco deve ser maior que o valor total do pedido!")


		if(ENTREGA){
			alert("Atenção! - Total com entrega: R$ " + TOTAL.toFixed(2))

		} else{
			alert("Atenção! - Total: R$ " + TOTAL.toFixed(2))
		} 
	}
	...
	getInstallment((installment) => {
		...
		.done(function(data){
			...
			if(data.consulta.original.status == "3"){
				...
				alert("Sucesso!", "Pagamento Aprovado!", "success")
		...
		.fail(function(err){
			...
			if(err.status == 403){
				...
				alert("Atenção!", "403: Erro de pagamento!", "warning")

			}else if(err.status == 404){
				json = err.responseJSON;
				alert("Atenção!", "404: Pagamento não autorizado!", "warning")
			}
			else if(err.status == 402){
				json = err.responseJSON;
				console.log(json)
				alert("Atenção!", "402: Pagamento ainda não aprovado pelo getway!", "warning")
