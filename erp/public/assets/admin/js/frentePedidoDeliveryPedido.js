...
$('#btn-add-sabor').click(() => {
	...
	validaSaborNaoAdicionado(sabor, (res) => {
		if(!res){
			...
			alert("Alerta", "Este sabor já esta adicionado, ou máximo de sabores escolhidos!!", "warning")
		}
	})
})
...
$('#btn-add-adicional').click(() => {
	...
	validaAdicionalNaoAdicionado(adicional, (res) => {
		if(!res){
			...
			alert("Alerta Esta adicional já esta escolhido!!", "warning")
		}
	})
})

$('#btn-add-adicional2').click(() => {
	let adicional = $('#kt_select2_3').val();
	validaAdicionalNaoAdicionado(adicional, (res) => {
		if(!res){
			ADICIONAISESCOLHIDOS.push(adicional)
			$('#adicioanis_escolhidos').val(ADICIONAISESCOLHIDOS)
			montaAdicionais((html) => {
				...
				$('#adicioanais-html2').html(html)
				$('#adicioanais-html2').css('display', 'block')
...
function montaAdicionais(call){
	...
	ADICIONAISESCOLHIDOS.map((s) => {
		ADICIONAIS.map((a) => {
			if(s == a.id){
				somaAdicional += parseFloat(a.valor);
				console.log(a)
				html += '<div class="col-sm-4 col-lg-4 col-6">';
