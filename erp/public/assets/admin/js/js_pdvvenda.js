...
function listarCaixaPorNumero(){
	 var numero_id = $("#numero_id").val();
	$.ajax({
         url: base_url + "admin/caixa/buscarCaixaPorNumero/" + numero_id,
         type: "get",
         dataType:"Json",
         data:{},
         success: function(data){
...
function atualizaTotalCompra() {
	...
	TOTAL_VENDA = parseFloat(total_da_venda  ).toFixed(2);
...
function salvarVenda() {
	...
	if( (caixa_id == '--') || (caixa_id == '') ||(caixa_id == 'null') ||(caixa_id == null)) {
		fecharModal();
		$("#mostrarUmErro").html(MostrarUmaMsgErro("Selecione uma caixa para continuar!"));	
		return false;	
	}
	...
	({
		...
		url: base_url + 'admin/pdvvenda/salvar',
		dataType: 'json',
		beforeSend: function (){
		   giraGira();
	   },
