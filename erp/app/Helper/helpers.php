...
//https://github.com/lipespry/moeda-php-classe/blob/master/Moeda.php
...
function listaIntevaloData($data_inicial, $data_final){   
    $retorno = array();
    $i=0;
    do {
        $nova_data= somarData($data_inicial, $i);
        $retorno[] = $nova_data;
        $i++;
    } while ($nova_data < $data_final);
...
function primeiroDiaSemana(){
    return date('l - Y-m-d', strtotime("this week"));    
}
function proximaSemana(){
    return date('l - Y-m-d', strtotime("sunday 0 week"));
}
...
function diferencaDataEmDias($data1, $data2){
...
function nomeSobrenome($nome){
    ...
    $retorno->nome 		= array_shift($partes);
    $retorno->sobrenome = $partes;
    return  $retorno;
}
function slug($str){
    # special accents
...
function calcularPrecoVenda($preco_custo, $margem){
    return $preco_custo/(1-$margem * 0.01);
}

function validarGrade($qtde_item, $nome_compo , $id_campo){
    $total_grade = GradeMovimento::where([$nome_compo => $id_campo])->sum("qtde_movimento");
   
    return $total_grade != $qtde_item ? false : true;
}

function validarGradeTemp($qtde_item, $nome_compo , $id_campo){
    $total_grade = GradeMovimentoTemp::where([$nome_compo => $id_campo])->sum("qtde_movimento");
    
    return $total_grade != $qtde_item ? false : true;
}

function verificarEValidarProdutoGrade($produto){
    $retorno = true;
    if($produto->usa_grade == "S"){
        $estoque        = $produto->estoque->quantidade ?? 0;
        $estoque_grade  = $produto->estoque->qtde_grade ?? 0;
        
        if ($estoque != $estoque_grade){
            $retorno = false;
        }        
    }    
    return $retorno;
...
function verificarEstoqueNegativo($produto, $qtde){
    $retorno        = true;    
    $estoque        = $produto->estoque->quantidade ?? 0;
