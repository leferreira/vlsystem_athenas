...
class ItemNotafiscalOperacaoService{
    ...
    public static function salvarItemDaNota($dados, $id){
        ...
        $item->desconto_item        = isset($dados->desconto_item) ?  getFloat($dados->desconto_item): 0       ;
        $item->desconto_rateio      = isset($dados->desconto_rateio) ?  getFloat($dados->desconto_rateio): 0          ;
        ...
        $item->tipo_calc_ipi        = isset($dados->tipo_calc_ipi) ?  getFloat($dados->tipo_calc_ipi): null            ;
        
        $item->cstCOFINS            = $dados->cstCOFINS          ;
        ...
        $item->vCOFINSST            = isset($dados->vCOFINSST) ?  getFloat($dados->vCOFINSST): null          ;        
        ...
        $item->vbc_somente_produto  =  isset($dados->vbc_somente_produto) ? "S" : "N" ;
        $item->vbc_frete            =  isset($dados->vbc_frete) ? "S" : "N" ;
        $item->vbc_ipi              =  isset($dados->vbc_ipi) ? "S" : "N" ;
        $item->vbc_outros           =  isset($dados->vbc_outros) ? "S" : "N" ;
        $item->vbc_seguro           =  isset($dados->vbc_seguro) ? "S" : "N" ;
        $item->vbc_desconto         =  isset($dados->vbc_desconto) ? "S" : "N" ;
        
        $item->ipi_somente_produto  =  isset($dados->ipi_somente_produto) ? "S" : "N" ;
        $item->ipi_frete            =  isset($dados->ipi_frete) ? "S" : "N" ;
        $item->ipi_outros           =  isset($dados->ipi_outros) ? "S" : "N" ;
        $item->ipi_seguro           =  isset($dados->ipi_seguro) ? "S" : "N" ;
        $item->ipi_desconto         =  isset($dados->ipi_desconto) ? "S" : "N" ;
        
        $item->pis_somente_produto  =  isset($dados->pis_somente_produto) ? "S" : "N" ;
        $item->pis_frete            =  isset($dados->pis_frete) ? "S" : "N" ;
        $item->pis_ipi              =  isset($dados->pis_ipi) ? "S" : "N" ;
        $item->pis_outros           =  isset($dados->pis_outros) ? "S" : "N" ;
        $item->pis_seguro           =  isset($dados->pis_seguro) ? "S" : "N" ;
        $item->pis_desconto         =  isset($dados->pis_desconto) ? "S" : "N" ;
        
        $item->cofins_somente_produto  =  isset($dados->cofins_somente_produto) ? "S" : "N" ;
        $item->cofins_frete         =  isset($dados->cofins_frete) ? "S" : "N" ;
        $item->cofins_ipi           =  isset($dados->cofins_ipi) ? "S" : "N" ;
        $item->cofins_outros        =  isset($dados->cofins_outros) ? "S" : "N" ;
        $item->cofins_seguro        =  isset($dados->cofins_seguro) ? "S" : "N" ;
        $item->cofins_desconto      =  isset($dados->cofins_desconto) ? "S" : "N" ; 
        
        $item->cst900_icms          =  isset($dados->cst900_icms) ? "S" : "N" ;
        $item->cst900_redbc         =  isset($dados->cst900_redbc) ? "S" : "N" ;
        $item->cst900_credisn       =  isset($dados->cst900_credisn) ? "S" : "N" ;
        $item->cst900_st            =  isset($dados->cst900_st) ? "S" : "N" ;
        $item->cst900_redbcst       =  isset($dados->cst900_redbcst) ? "S" : "N" ; 
