...
class BaseCalculoService{
    public static function calculoVbcIcms($item, $tributacao){
        ...
        //Para não destacar ICMS
        if($item->cstICMS=="102" || $item->cstICMS=="103" || $item->cstICMS=="300" || $item->cstICMS=="400" ){
            $item->destaca_icms = 'N';
        }
    ...
    public static function calculoVbcCofins($item, $tributacao){
        $vBC = $item->vProd;
        if($item->cofins_somente_produto=="S"){
            $vBC = $item->vUnCom;
        }
