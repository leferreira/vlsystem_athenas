...
use App\Service\ItemNfceService;
...
class ItemNfceObserver
{
    ...
    public function created(NfceItem $item){
       // echo "item - created<br>";
        ...
        $nfce->save();        
        ItemNfceService::refazTodosCalculos($nfce);
        ItemNfceService::atualizarTotaisImpostosDaNota($nfce->id);
     }
      
   
    public function deleted(NfeItem $item){ 
        $nfe = Nfe::find($item->nfe_id);      
        ItemNfceService::refazTodosCalculos($nfe);
        ItemNfceService::atualizarTotaisImpostosDaNota($nfe->id );
    }
