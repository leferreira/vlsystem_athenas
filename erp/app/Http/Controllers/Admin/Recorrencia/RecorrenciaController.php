...
class RecorrenciaController extends Controller{    
    ...
    public function index(){     
        ...
        $dados["vendas"]               = VendaRecorrente::get();       
        $dados["vencidos"]             = Cobranca::where("data_vencimento", "<", hoje())->limit(10)->get();
        $dados["vencimentos_proximo"]  = Cobranca::where("data_vencimento", ">=", hoje())->where("data_vencimento", "<", somarData(hoje(),5))->limit(10)->get();
        $dados["rececimentos"]         = FinRecebimento::orderBy("id", "desc")->limit(10) ->get();
        
        $dados["clientes"]             = VendaRecorrente::select('cliente_id')->distinct()->get();
        
        return view("Admin.Recorrencia.Dashboard", $dados);   
