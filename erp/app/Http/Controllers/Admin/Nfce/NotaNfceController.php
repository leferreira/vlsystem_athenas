...
{    
    ...
    public function create(){
        ...
        if(!$emitente->numero_serie_nfce || !$emitente->ultimo_numero_nfce){
            return redirect()->back()->with('janela_atencao1', "Configure a série e o último Número da NFCE.");
        }
        ...
        return view("Admin.Nfce.Novo", $dados);
    }
    
    //Cria uma nova Nota
    public function criarNota(NovaNfceRequest $request){
    ...
    public function configurarProdutoNfe($id)  {
        $dados["pode_editar"]   = NfeItem::where("produto_id", Null)->first();
    ...
    public function edit($id){         
        ...
        $dados["notafiscal"]        = $nfce;
        $dados["destinatario"]      = NfeDestinatario::where("nfe_id", -1)->first();
        $dados["itens"]             = NfceItem::where("nfce_id", $id)->get();
        $dados["duplicatas"]        = NfceDuplicata::where("nfce_id",$id)->get();
        $dados["referenciados"]     = NfceReferenciado::where("nfce_id", $id)->get();
        ...
        $dados["xml"]               = NfceXml::where("nfce_id", $id)->first();
      
        return view("Admin.Nfce.Edit", $dados);
    }
