...
class MovimentoGradeController extends Controller
{
    ...
    {      
        ...
        $retorno  = new \stdClass();
        try {
            if($request->campo=="item_venda_id"){
                $item        = ItemVenda::find($request->item_id);
            }else if($request->campo=="item_compra_id"){
                $item        = ItemCompra::find($request->item_id);
            }
                       
            $grade_produto = GradeProduto::find($request->grade_id); 
            
            
            //Verificar o Estoque é maior que a saida
            $estoque_grade = $grade_produto->estoque_temporario;
            if($request->campo=="item_venda_id"){                
                if($estoque_grade - $request->qtde < 0){
                    throw new \Exception('Quantidade maior que o estoque');
                }
            }
            //Verifica se já existe um movimento
            $temGradeMovimento = GradeMovimento::where([$request->campo => $request->item_id, "grade_id" =>$request->grade_id])->first();
            ...
            $soma = GradeMovimento::where($request->campo, $request->item_id)->sum('qtde_movimento');            
            ...
            if($request->campo=="item_venda_id"){
                $retorno->estoque_grade_item = $estoque_grade - $request->qtde;
            }else{
                $retorno->estoque_grade_item = $estoque_grade + $request->qtde;
            }
    ...
    public function inserirAjusteGrade(Request $request)
    {
        ...
        try {
            ...
            $mov                    = new \stdClass();
            if($request->tipo=="E"){
                $mov->tipo_movimento_id = config("constantes.tipo_movimento.ENTRADA_AJUSTE_BALANCO");
                $mov->ent_sai           = 'E';
            }
            
            if($request->tipo=="S"){
                $mov->tipo_movimento_id = config("constantes.tipo_movimento.SAIDA_AJUSTE_BALANCO");
                $mov->ent_sai           = 'S';
            }
            
            $mov->produto_id        = $grade->produto_id;
            $mov->grade_id          = $request->grade_id;
            $mov->estorno           = 'N';
            $mov->data_movimento    = hoje();
            $mov->qtde_movimento    = $request->qtde;               
            $mov->descricao         = "Ajuste de Estoque " ;           
    ...
    public function destroy( $id){
        try {
            $mov = GradeMovimento::find($id);            
            $mov->delete();
            
            $lista          = GradeMovimento::where("item_venda_id", $mov->item_venda_id)->get();
            $movimentos     = array();
            foreach($lista as $l){
                $grade              = $l->grade;
                $movimento          = new \stdClass();
            ...
            $retorno = new \stdClass();
            $retorno->qtde_movimento = GradeMovimento::where("item_venda_id", $mov->item_venda_id)->sum('qtde_movimento');
            $retorno->movimentos   = $movimentos;
