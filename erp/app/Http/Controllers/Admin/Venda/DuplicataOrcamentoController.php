...
class DuplicataOrcamentoController extends Controller{ 
      
    public function inserir(Request $request){
        $req            = $request->except(["_token","_method"]);  
        $orcamento            = Orcamento::find($req["orcamento_id"]);
        ...
        $valor_total    = $orcamento->valor_liquido;
        $qtde_parcela   = count($array_parcela);
        $parcela        = number_format($valor_total / $qtde_parcela, 2, ".", "");

        $soma_parcela          = 0;
        Duplicata::where("orcamento_id", $req["orcamento_id"])->delete();
        for($i=0; $i < $qtde_parcela; $i++){
            $duplicata          = new \stdClass();            
            
            $duplicata->orcamento_id  = $req["orcamento_id"];
            ...
            $duplicata->vDup = $valor_parcela;
             Duplicata::Create(objToArray($duplicata));
            
        }    
        
        $lista = $this->listarDuplicata($req["orcamento_id"]);
      ...
      public function salvarAlteracao(Request $request){
          $req    = $request->except(["_token","_method"]);
          Duplicata::where("id", $req["id"])->update($req);         
      ...
      public function alterar(Request $request){
          $req     = $request->except(["_token","_method"]);
          $qtde = Duplicata::where("orcamento_id", $req["orcamento_id"])->count("orcamento_id");
      ...
      public function listarDuplicata($orcamento_id){
          $lista = Duplicata::where("orcamento_id", $orcamento_id)->get();
          $resultado = array();
          foreach($lista as $l){
              $duplicata          = new \stdClass();
              $duplicata->id      = $l->id;
              $duplicata->orcamento_id  = $l->orcamento_id;
      ...
      public function excluir($id){ 
          try {
              $duplicata    = Duplicata::where("id", $id)->first();
              $valor_total  = Orcamento::find($duplicata->orcamento_id)->valor_orcamento;
             // $valor_total  = Duplicata::where("orcamento_id", $duplicata->orcamento_id)->sum("vDup");
              //exclui o item
              $duplicata->delete();
              $lista      = Duplicata::where("orcamento_id", $duplicata->orcamento_id)->get();
      ...
      public function salvar(Request $request){
          $req     = $request->except(["_token","_method"]);
          $req["nDup"] = 1;
          Duplicata::Create($req);
          $lista = Duplicata::where("orcamento_id", $req["orcamento_id"])->get();
          echo json_encode($lista);
