...
<span class="p-2 bg-title text-light text-uppercase text-branco justify-content-space-between d-flex">
	<span class="h5 mb-0"><i class="fas fa-plus-circle"></i> Cobrança Num: {{$cobranca->id}}</span>
	<div class="d-flex">
		<a href="{{route('admin.cobranca.index')}}" class="btn btn-azul btn-pequeno" title="Voltar"><i class="fas fa-arrow-left"></i> </a>
		<a href="" class="retorna btn btn-roxo btn-pequeno ml-1 d-inline-block" title="Menu"><i class="fas fa-bars"></i></a>
	</div>
</span>
<div class="rows">
	<div class="col-12 mt-3">
    	<form action="{{route('admin.cobranca.update', $cobranca->id)}}" method="POST" >
				...
				<div class="rows pt-3 pb-4">					
                    ...
                    <div class="col-6 mb-3">
						<label class="text-label">Descrição da Cobranca</label>
						 <input type="text" name="descricao"  id="descricao" value="{{$cobranca->descricao}}"  class="form-campo">												
					</div>
					
					<div class="col-6 mb-3">
						<label class="text-label">Cliente</label>
						 <input type="text" readonly  value="{{$cobranca->cliente->nome_razao_social}}"  class="form-campo">												
					</div>
					...
					<div class="col-4 mb-3">
						<label class="text-label">Data Emissão</label>
						 <input type="date" readonly value="{{$cobranca->data_cadastro ?? null }}"  class="form-campo">												
					</div>	
					
										
					<div class="col-4 mb-3">
						<label class="text-label">Valor</label>
						<input type="text" name="valor" value="{{$cobranca->valor}}"  class="form-campo mascara-float">												
					</div>			
                        
                        
                        <div class="col-4 validated">	
                            <label class="text-label d-block ">Data Vencimento</label>
                            <input type="date" name="data_vencimento" value="{{$cobranca->data_vencimento}}"  class="form-campo ">
                        </div>
