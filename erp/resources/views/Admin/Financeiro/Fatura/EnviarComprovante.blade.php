            ...
            <div class="caixa">
				...
				<div class="rows pt-3 pb-4">
					<div class="col-4">
						...
						<select name="forma_pagto_id" class="form-campo">
							@foreach(config('constantes.forma_pagto') as $chave =>$valor)
								<option value="{{$valor}}" {{$valor==config('constantes.forma_pagto.DEPOSITO_BANCARIO') ? 'selected': ''}}>{{$valor}} - {{$chave}}</option>
							@endforeach
						</select>
					</div>
					...
					<div class="col-2 mb-3">
						...
						<input type="text" name="valor_pago" id="valor_pago" required="required" value="{{ formataNumeroBr($fatura->valor) }}"    class="form-campo mascara-float">												
					</div> 
					...
					<div class="col-4 mb-3">
					 ...
					 <label class="text-label">.</label> 
						...
						<input type="hidden" value="Pagamento de Fatura"  name="descricao"  >
