...
<div class="rows">	
	...
	<div class="p-2  bg-title text-light text-uppercase  text-branco d-flex justify-content-space-between">
		<span class="mb-0 h5"><i class="fas fa-cog"></i> Lista de bancos</span>
		<div class="d-flex">		
		 ...
		 @if(isset($banco))    
           <form action="{{route('admin.banco.update', $banco->id)}}" method="POST" >
           <input name="_method" type="hidden" value="PUT"/>
         @else                       
        	<form action="{{route('admin.banco.store')}}" method="Post" >
        @endif
        	@csrf
            
            <div class="px-2 pt-2">	
				  <div class=" {{isset($banco) ? 'bg-orange' : 'bg-padrao' }}  mt-2 p-2 radius-4">
				  <div class="rows">
                            <div class="col-2">	
                                    <label class="text-label d-block text-branco">Código </label>
                                    <input type="text" name="codigo" value="{{isset($banco->codigo) ? $banco->codigo : old('codigo')}}"  class="form-campo">
                            </div>  
                            
                            <div class="col-8">	
                                    <label class="text-label d-block text-branco">Nome </label>
                                    <input type="text" name="banco" value="{{isset($banco->banco) ? $banco->banco : old('banco')}}"  class="form-campo">
                            </div>                                   
                            
                            <div class="col-2 mt-1 pt-1">
                                    <input type="submit" value="{{isset($banco) ? 'Alterar' : 'Inserir' }}" class="width-100 btn btn-roxo text-uppercase">
                            </div>
                    </div>
                    </div>
            </div>
                    ...
                    <table cellpadding="0" cellspacing="0" id="dataTable" width="100%">
                          ...
                          @foreach($bancos as $cat)                                     
                             <tr>
                                ...
                                <td align="left">{{$cat->banco}}</td>											
                                <td align="center"><a href="{{route('admin.banco.edit', $cat->id)}}" class="d-inline-block btn btn-circulo btn-verde btn-pequeno" title="Editar"><i class="fas fa-edit"></i></a>  </td>									
                                <td align="center">
                                ...
                                <a href="#" onclick="confirm('Tem Certeza?') ? document.getElementById('apagar{{$cat->id}}').submit() : '';" class="d-inline-block btn btn-circulo btn-vermelho btn-pequeno" title="Ecluir"><i class="fas fa-trash-alt"></i></a>
                                    <form action="{{route('admin.banco.destroy', $cat->id)}}" method="POST" id="apagar{{$cat->id}}">
		...
		<div class="col-2 MostraOpcoes sm-mx" id="opcoes_cliente">
			...
			<ul class="cx-opcoes" >
				<li class="titulo bg-padrao text-branco d-flex center-middle justify-content-space-between">
					<span>Banco: <span id="id_cliente"></span> </span> 
