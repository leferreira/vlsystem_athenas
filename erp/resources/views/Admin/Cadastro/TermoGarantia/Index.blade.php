...
<div class="rows">	
           ...
           <div class="p-2 py-1 bg-title text-light text-uppercase text-branco d-flex justify-content-space-between">
				<span class="d-flex center-middle h5 mb-0 "><i class="far fa-list-alt mr-1"></i> Lista de termogarantia </span>
				<div>
					<a href="{{route('admin.termogarantia.create')}}" class="btn btn-azul ml-1 d-inline-block" title="Adicionar novo"><i class="fas fa-plus-circle"></i> </a>
					...
					<div class="mostraFiltro bg-padrao mt-2 p-2 radius-4">
						 <div class="rows p-3">	
							<div class="col-6">
									...
									<input type="text" name="termogarantia" value="" placeholder="Digite aqui..." class="form-campo">
							</div>
							<div class="col-4">
    ...
    <div class="col">
            ...
            <table cellpadding="0" cellspacing="0" id="dataTable" width="100%">
                    <thead>
                            <tr>
                                    ...
                                    <th align="left">Referênca</th>
                                    <th align="center">Data Garantia</th>
                    ...
                    @foreach($lista as $l)
						<tr>
							...
							<td align="left">{{$l->referencia_garantia}}</td>
							<td align="center">{{$l->data_garantia}}</td>
							<td align="center"><a href="{{route('admin.termogarantia.edit', $l->id)}}" class="d-inline-block btn btn-verde btn-circulo btn-pequeno" title="Editar"><i class="fas fa-edit"></i></a></td>
							 <td align="center">
							<a href="#" onclick="confirm('Tem Certeza?') ? document.getElementById('apagar{{$l->id}}').submit() : '';" class="d-inline-block btn btn-vermelho btn-circulo btn-pequeno" title="Excluir"><i class="fas fa-trash-alt"></i></a>
                                    <form action="{{route('admin.termogarantia.destroy', $l->id)}}" method="POST" id="apagar{{$l->id}}">
					...
					@endforeach
								...
								<div class="caixa p-2">
									...
									<p><b><i class="fas fa-check"></i> Mensagem de boas vindas</b> Parabéns seu termogarantia foi inserido com sucesso</p>
									</div>
		...
		<div class="col-2 MostraOpcoes sm-mx" id="opcoes_termogarantiaes">
		
			<ul class="cx-opcoes" >
				<li class="titulo bg-padrao text-branco d-flex center-middle justify-content-space-between">
					<span>Fornecedor: <span id="id_termogarantia"></span> </span> 
					<a href="javascript:;" onClick="fechar_opcoes_termogarantiaes()" title="Fechar Opções" class="text-vermelho position-inherit p-0"><i class="fas fa-times position-inherit"></i></a>
				</li>
...
<script>
var termogarantia_id = 0;
function abrir_opcoes_termogarantiaes(id){
	termogarantia_id = id;
	$("#id_termogarantia").html(id);
	mostrar_opcoes('opcoes_termogarantiaes')
	
}

function verMovimento(){	
	giraGira();
	location.href = base_url + "admin/termogarantia/movimento/" + termogarantia_id;
}

function editar(){	
	giraGira();
	location.href = base_url + "admin/termogarantia/" + termogarantia_id + "/edit";
}
...
function fechar_opcoes_termogarantiaes(){
	esconder_opcoes('opcoes_termogarantiaes');
}
</script>
@endsection