...
<div class="p-2 bg-title text-light text-uppercase  text-branco d-flex justify-content-space-between">
		<span class="mb-0 h5"><i class="fas fa-cog"></i> Detalhes do Item da Nota</span>
		 		
	</div>
     
   <form action="{{route('admin.itemnotafiscal.atualizar')}}" method="POST">
    @csrf
					...
					<div class="col-6 mb-3">
							...
							<select class="form-campo" name="cstICMS" id="cstICMS" onchange="selecionarIcms()" >							
								@foreach($lista_cstIcms as $chave=>$valor)
									<option value="{{$chave}}" {{($nfeItem->cstICMS ?? null) == $chave ? "selected" : null }}>{{$chave}} - {{$valor}}</option>
								@endforeach
							</select>
					...
					<div class="col-6 mb-3">
							...
							<select class="form-campo" name="cstIPI" id="cstIPI" >							
								@foreach(ConstanteService::listaCST_IPI() as $chave=>$valor)
									<option value="{{$chave}}" {{($nfeItem->cstIpi ?? null) == $chave ? "selected" : null }}>{{$chave}} - {{$valor}}</option>
								@endforeach
							</select>		
					...
					<div class="col-12 mb-3">
							...
							<select class="form-campo" name="cstPIS" id="cstPIS" >							
								@foreach(ConstanteService::listaCST_PIS_COFINS() as $chave=>$valor)
									<option value="{{$chave}}" {{($nfeItem->cstCofins ?? null) == $chave ? "selected" : null }}>{{$chave}} - {{$valor}}</option>
								@endforeach
							</select>		
