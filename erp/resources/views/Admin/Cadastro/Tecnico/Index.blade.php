...
<div class="rows">	
           ...
           <div class="p-2 py-1 bg-title text-light text-uppercase text-branco d-flex justify-content-space-between">
				<span class="d-flex center-middle  h5 mb-0"><i class="far fa-list-alt mr-1"></i> Lista de tecnico </span></span>
				<div>
					<a href="{{route('admin.tecnico.create')}}" class="btn btn-azul ml-1 d-inline-block" title=" Adicionar novo"><i class="fas fa-plus-circle"></i></a>
				...
				<div class="px-2 pt-2"> 
					...
					<div class="mostraFiltro bg-padrao mt-2 p-2 radius-4">
						 <div class="rows p-3">	
							<div class="col-6">
									<span class="text-label text-branco">Tecnico</span>
									<input type="text" name="tecnico" value="" placeholder="Digite aqui..." class="form-campo">
							</div>
							<div class="col-4">
    ...
    <div class="col">
            ...
            <table cellpadding="0" cellspacing="0" id="dataTable" width="100%">
                    <thead>
                            <tr>
                                    ...
                                    <th align="left" width="380">Nome</th>
                    ...
                    @foreach($lista as $l)
						<tr>
							...
							<td align="center">{{$l->cpf}}</td>
							<td align="center">{{$l->email}}</td>
							<td align="center">{{$l->celular}}</td>
							<td align="center"><a href="{{route('admin.tecnico.edit', $l->id)}}" class="d-inline-block btn btn-verde btn-circulo btn-pequeno" title="Editar"><i class="fas fa-edit"></i></td>
							 <td align="center">
							<a href="#" onclick="confirm('Tem Certeza?') ? document.getElementById('apagar{{$l->id}}').submit() : '';" class="d-inline-block btn btn-vermelho btn-circulo btn-pequeno" title="Excluir"><i class="fas fa-trash-alt"></i></a>
                                    <form action="{{route('admin.tecnico.destroy', $l->id)}}" method="POST" id="apagar{{$l->id}}">
                                        <input type="hidden" name="_method" value="DELETE">
                                        {{csrf_field()}}
                                    </form>
                                 </td>	
                            <td align="center">
										<a href="javascript:;" onclick="abrir_opcoes_tecnico({{$l->id}})" ><i class="ellipsis-vertical"></i></a>
							</td>					
                            </tr>
					@endforeach
					</tbody>
		...
		<div class="col-2 MostraOpcoes sm-mx" id="opcoes_tecnico">
		
			<ul class="cx-opcoes" >
				<li class="titulo bg-padrao text-branco d-flex center-middle justify-content-space-between">
					<span>Tecnico: <span id="id_tecnico"></span> </span> 
					<a href="javascript:;" onClick="fechar_opcoes_tecnico()" title="Fechar Opções" class="text-vermelho position-inherit p-0"><i class="fas fa-times position-inherit"></i></a>
				</li>
				
				<li><a href="javascript:;" onClick="verMovimento()" title="Gerar Nota Fiscal"><i class="fas fa-layer-group"></i> Movimentações do Tecnico</a></li>
...
<script>
var tecnico_id = 0;
function abrir_opcoes_tecnico(id){
	tecnico_id = id;
	$("#id_tecnico").html(id);
	mostrar_opcoes('opcoes_tecnico')
	
}

function verMovimento(){	
	giraGira();
	location.href = base_url + "admin/tecnico/movimento/" + tecnico_id;
}

function editar(){	
	giraGira();
	location.href = base_url + "admin/tecnico/" + tecnico_id + "/edit";
}
...
function fechar_opcoes_tecnico(){
	esconder_opcoes('opcoes_tecnico');
}
</script>
@endsection