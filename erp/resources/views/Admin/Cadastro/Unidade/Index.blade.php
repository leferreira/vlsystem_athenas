...
<div class="rows">	
	...
	<div class="p-2  bg-title text-light text-uppercase  text-branco d-flex justify-content-space-between">
		<span class="mb-0 h5"><i class="fas fa-cog"></i> Lista de unidades</span>
		<div class="d-flex">		
        ...
        @endif
            ...
            <div class="px-2 pt-2">	
				  <div class=" {{isset($unidade) ? 'bg-orange' : 'bg-padrao' }}  mt-2 p-2 radius-4">
				  <div class="rows">
                            <div class="col-10">	
                                    <label class="text-label d-block text-branco">Nome </label>
                                    <input type="text" name="unidade" value="{{isset($unidade->unidade) ? $unidade->unidade : null}}"  class="form-campo">
                            </div>                                    
                            
                            <div class="col-2 mt-1 pt-1">
                                    <input type="submit" value="{{isset($unidade) ? 'Alterar' : 'Inserir' }}" class="width-100 btn btn-roxo text-uppercase">
                            </div>
                    </div>
                    </div>
            </div>
                    ...
                    <table cellpadding="0" cellspacing="0" id="dataTable" width="100%">
                          ...
                          @foreach($unidades as $cat)                                     
                             <tr>
                                ...
                                <td align="left">{{$cat->unidade}}</td>											
                                <td align="center"><a href="{{route('admin.unidade.edit', $cat->id)}}" class="d-inline-block btn btn-circulo btn-verde btn-pequeno" title="Editar"><i class="fas fa-edit"></i></a>  </td>									
                                <td align="center">
                                ...
                                <a href="#" onclick="confirm('Tem Certeza?') ? document.getElementById('apagar{{$cat->id}}').submit() : '';" class="d-inline-block btn btn-circulo btn-vermelho btn-pequeno" title="Ecluir"><i class="fas fa-trash-alt"></i></a>
                                    <form action="{{route('admin.unidade.destroy', $cat->id)}}" method="POST" id="apagar{{$cat->id}}">
