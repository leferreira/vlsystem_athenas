...
<div class="rows">	
           ...
           <div class="p-2 py-1 bg-title text-light text-uppercase text-branco d-flex justify-content-space-between">
				<span class="d-flex center-middle  h5 mb-0"><i class="far fa-list-alt mr-1"></i> Lista de vendedor </span></span>
				<div>
					<a href="{{route('admin.vendedor.create')}}" class="btn btn-azul ml-1 d-inline-block" title=" Adicionar novo"><i class="fas fa-plus-circle"></i></a>
				</div>
			</div>
    ...
    <div class="col">
            ...
            <table cellpadding="0" cellspacing="0" id="dataTable" width="100%">
                    ...
                    @foreach($lista as $l)
						<tr>
							...
							<td align="center">{{$l->comissao}}</td>
							<td align="center"><a href="{{route('admin.vendedor.edit', $l->id)}}" class="d-inline-block btn btn-verde btn-circulo btn-pequeno" title="Editar"><i class="fas fa-edit"></i></td>
							 <td align="center">
							<a href="#" onclick="confirm('Tem Certeza?') ? document.getElementById('apagar{{$l->id}}').submit() : '';" class="d-inline-block btn btn-vermelho btn-circulo btn-pequeno" title="Excluir"><i class="fas fa-trash-alt"></i></a>
                                    <form action="{{route('admin.vendedor.destroy', $l->id)}}" method="POST" id="apagar{{$l->id}}">
                                        <input type="hidden" name="_method" value="DELETE">
                                        {{csrf_field()}}
                                    </form>
                                 </td>	
                            <td align="center">
										<a href="javascript:;" onclick="abrir_opcoes_vendedor({{$l->id}})" ><i class="ellipsis-vertical"></i></a>
							</td>					
                            </tr>
					@endforeach
					</tbody>
		...
		<div class="col-2 MostraOpcoes sm-mx" id="opcoes_vendedor">
		
			<ul class="cx-opcoes" >
				<li class="titulo bg-padrao text-branco d-flex center-middle justify-content-space-between">
					<span>Vendedor: <span id="id_vendedor"></span> </span> 
					<a href="javascript:;" onClick="fechar_opcoes_vendedor()" title="Fechar Opções" class="text-vermelho position-inherit p-0"><i class="fas fa-times position-inherit"></i></a>
				</li>
				
				<!--<li><a href="javascript:;" onClick="verMovimento()" title="Gerar Nota Fiscal"><i class="fas fa-layer-group"></i> Movimentações do Vendedor</a></li>-->
...
<script>
var vendedor_id = 0;
function abrir_opcoes_vendedor(id){
	vendedor_id = id;
	$("#id_vendedor").html(id);
	mostrar_opcoes('opcoes_vendedor')
	
}

function verMovimento(){	
	giraGira();
	location.href = base_url + "admin/vendedor/movimento/" + vendedor_id;
}

function editar(){	
	giraGira();
	location.href = base_url + "admin/vendedor/" + vendedor_id + "/edit";
}
...
function fechar_opcoes_vendedor(){
	esconder_opcoes('opcoes_vendedor');
}
</script>
@endsection