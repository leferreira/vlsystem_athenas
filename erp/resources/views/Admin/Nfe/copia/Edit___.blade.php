   ...
   <div id="tab">
	  ...
	  <div id="tab-1">
		<div class="p-2">
                ...
                <input type="text" name="numero_nfe" value="{{ $nota->nNF }}" class="form-campo">
    	</div>				

		<div class="col-2 mb-3">
                    <label class="text-label">Tipo de operação</label>	
                    <select class="form-campo" name="tpNF" id="tpNF" onchange="selecionar_tipo_operacao();">
                            <option value="0" {{ ($nota->tpNF=='0') ? "selected": NULL }} >ENTRADA</option>
                            <option value="1" {{ (($nota->tpNF=='1') || (!$nota->tpNF)) ? "selected": NULL }}>SAÍDA</option>
                    </select>
		</div>
		<div class="col-6 mb-3">
                <label class="text-label">Natureza Operação</label>	
                <input type="text" name="numero_nfe" value="{{ $nota->natOp }}" class="form-campo">  
		</div>					
		<div class="col-2 mb-3">
                <label class="text-label">Ambiente</label>	
                <select class="form-campo" name="tpAmb" id="tpAmb">
                        <option value="1" {{ ($nota->tpAmb =="1") ? "selected": NULL }} >Produção</option>
                        <option value="2" {{ ($nota->tpAmb =="2") ? "selected": NULL }} >Homologação</option>                                                
				</select>
        </div>
		
                <div class="col-3 mb-3">
                    <label class="text-label">Data Emissão NF</label>	
                    <input type="date" name="data_emissao_nfe" value="{{ ($nota->dhEmi)? dataNfe($nota->dhEmi): date('Y-m-d') }}"  placeholder="Digite aqui..." class="form-campo">
                </div>
                <div class="col-2 mb-3">
                    <label class="text-label">Hora emissão NF</label>	
                    <input type="time" name="hora_emissao_nfe" value="{{ ($nota->dhEmi) ? horaNfe($nota->dhEmi): date('H:i:s') }}" placeholder="Digite aqui..." class="form-campo">
                </div>                             
	
                <div class="col-3 mb-3">
                    <label class="text-label">Data saída/entrada</label>	
                    <input type="date" name="data_saida_nfe" value="{{ ($nota->dhSaiEnt)? dataNfe($nota->dhSaiEnt):date('Y-m-d') }}" placeholder="Digite aqui..." class="form-campo">
                </div> 
                <div class="col-2 mb-3">
                    <label class="text-label">Hora saída/entrada</label>	
                    <input type="time" name="hora_saida_nfe" value="{{ ($nota->dhSaiEnt)? horaNfe($nota->dhSaiEnt):date('H:i:s') }}"  placeholder="Digite aqui..." class="form-campo">
                </div>                		
                ...
                <div class="col-2 mb-3">
                        ...
                        <select class="form-campo" name="indFinal" id="indFinal">
                                <option value="0" {{ ($nota->indFinal =="0") ? "selected": NULL }}>NÃO</option>
                                <option value="1" {{ ($nota->indFinal =="1") ? "selected": NULL }}>SIM</option>
                        </select>
                </div>
                ...
                <select class="form-campo" name="finNFe" id="finNFe">
                    <option value="1" {{ ($nota->finNFe =="1") ? "selected": NULL }}> NORMAL</option>
                    <option value="2" {{ ($nota->finNFe =="2") ? "selected": NULL }}>COMPLEMENTAR</option>
                    <option value="3" {{ ($nota->finNFe =="3") ? "selected": NULL }}>DE AJUSTE</option>
                    <option value="4" {{ ($nota->finNFe =="4") ? "selected": NULL }}>DEVOLUÇÃO DE MERCADORIA</option>
                </select>
                </div>
                ...
                <div class="col-4  mb-3">
                        ...
                        <select class="form-campo" name="indPres" id="indPres">
                            <option value="0" {{ ($nota->indPres =="0") ? "selected": NULL }}>NÃO SE APLICA</option>
                            <option value="1" {{ ($nota->indPres =="1") ? "selected": NULL }}>OPERAÇÃO PRESENCIAL</option>
                            <option value="2" {{ ($nota->indPres =="2") ? "selected": NULL }}>OPERAÇÃO NÃO PRESENCIAL, PELA INTERNET</option>
                            <option value="3" {{ ($nota->indPres =="3") ? "selected": NULL }}>OPERAÇÃO NÃO PRESENCIAL, TELEATENDIMENTO</option>
                            <option value="4" {{ ($nota->indPres =="4") ? "selected": NULL }}>NFC-e EM OPERAÇÃO COM ENTREGA A DOMICÍLIO</option>
                            <option value="5" {{ ($nota->indPres =="5") ? "selected": NULL }}>OPERAÇÃO PRESENCIAL, FORA DO ESTABELECIMENTO</option>
                            <option value="9" {{ ($nota->indPres =="9") ? "selected": NULL }}>OPERAÇÃO NÃO PRESENCIAL, OUTROS</option> 
                        </select>
                </div>                                    
	...
	<div class="rows p-2"> 
        ...
        <div class="rows">	
            ...
            <div class="col-2 mb-1">
                    ...
                    <input type="text" value="{{ $nota->vBC }}" readonly  class="form-campo">
            </div>	
            <div class="col-2 mb-1">
                    <label class="text-label">Total ICMS</label>	
                    <input type="text"  value="{{ $nota->vICMS }}" readonly  class="form-campo">
            </div>	
            <div class="col-2 mb-1">
                    <label class="text-label">Total ICMS desonerado</label>	
                    <input type="text"  value="{{ $nota->vICMSDeson }}" readonly class="form-campo">
            </div>
                
            <div class="col-2 mb-1">
                     <label class="text-label">Total do FCP</label>	
                     <input type="text"  value="{{ $nota->vFCP }}" readonly class="form-campo">
             </div>	
             <div class="col-2 mb-1">
                     <label class="text-label">Total BC ICMS ST</label>	
                     <input type="text"  value="{{ $nota->vBCST }}" readonly class="form-campo">
             </div>	
             <div class="col-2 mb-1">
                     <label class="text-label">Total ICMS ST</label>	
                     <input type="text"  value="{{ $nota->vST }}" readonly class="form-campo">
             </div>

            <div class="col-2 mb-1">
                    <label class="text-label">Total do FCPST</label>	
                    <input type="text"  value="{{ $nota->vFCPST }}" readonly class="form-campo">
            </div>
            <div class="col-2 mb-4">
                    <label class="text-label">Total do FCPST Ret.</label>	
                    <input type="text"  value="{{ $nota->vFCPSTRet }}" readonly class="form-campo">
            </div>          

            <div class="col-2 mb-1">
                    <label class="text-label">Total bruto dos produtos</label>	
                    <input type="text" value="{{ $nota->vProd }}" readonly class="form-campo">
            </div>	
            <div class="col-2 mb-1">
                    <label class="text-label">Total de frete</label>	
                    <input type="text"  value="{{ $nota->vFrete }}" readonly class="form-campo">
            </div>	
            <div class="col-2 mb-1">
                    <label class="text-label">Total de seguro</label>	
                    <input type="text"  value="{{ $nota->vSeg }}" readonly class="form-campo">
            </div>
                
            <div class="col-2 mb-1">
                    <label class="text-label">Total de desconto</label>	
                    <input type="text"  value="{{ $nota->vDesc }}" readonly class="form-campo">
            </div>	

            <div class="col-2 mb-1">
                    <label class="text-label">Total de II</label>	
                    <input type="text"  value="{{ $nota->vII }}" readonly class="form-campo">
            </div>
                
            <div class="col-2 mb-1">
                    <label class="text-label">Total de IPI</label>	
                    <input type="text" value="{{ $nota->vIPI }}" readonly class="form-campo">
            </div>
                
                <div class="col-2 mb-1">
                    <label class="text-label">Total de IPI Devolução</label>	
                    <input type="text"  value="{{ $nota->vIPIDevol }}" readonly  class="form-campo">
            </div>             
                
            <div class="col-2 mb-1">
                    <label class="text-label">Total de PIS</label>	
                    <input type="text"  value="{{ $nota->vPIS }}" readonly  class="form-campo">
            </div>
            <div class="col-2 mb-1">
                    <label class="text-label">Total de COFINS</label>	
                    <input type="text"  value="{{ $nota->vCOFINS }}" readonly  class="form-campo">
            </div>
                
            <div class="col-2 mb-1">
                    <label class="text-label">Total Outras Despesas</label>	
                    <input type="text"  value="{{ $nota->vOutro }}" readonly  class="form-campo">
            </div>
                
            <div class="col-2 mb-1">
                    <label class="text-label">TOTAL DA NF</label>	
                    <input type="text"  value="{{ $nota->vNF }}" readonly  class="form-campo">
            </div>
            <div class="col-2 mb-1">
                    <label class="text-label">Total Aproximado</label>	
                    <input type="text"  value="{{ $nota->vTotTrib }}" readonly  class="form-campo">
            </div>
...
<legend class="h5 mb-0 text-left">Veículo/Reboque/Balsa/Vagão</legend>
			<div class="rows p-2">				
				<div class="col-4 mb-3">
                    ...
                    <input type="text" name="veiculo_placa" id="veiculo_placa" value="<?php   echo ($transportadora->veiculo_placa) ?? null ?>" placeholder="Digite aqui..." class="form-campo">
				</div>
				<div class="col-4 mb-3">
				...
				<div class="col-4 mb-3">
                    ...
                    <input type="text" name="veiculo_RNTC" id="veiculo_RNTC" value="<?php   echo ($transportadora->veiculo_RNTC) ?? null ?>" placeholder="Digite aqui..." class="form-campo">
				</div>
				<div class="col-6 mb-3">
                    <label class="text-label">Identificação vagão</label>	                   
                    <input type="text" name="veiculo_vagao" id="veiculo_vagao" value="<?php   echo ($transportadora->veiculo_vagao) ?? null ?>" placeholder="Digite aqui..." class="form-campo">
				</div>
				<div class="col-6 mb-3">
                    <label class="text-label">Identificação balsa</label>	                   
                    <input type="text" name="veiculo_balsa" id="veiculo_balsa" value="<?php   echo ($transportadora->veiculo_balsa) ?? null ?>" placeholder="Digite aqui..." class="form-campo">
				</div>
			</div>	
</fieldset>
