...
<div class="rows">	
                ...
                <div class="caixa">
					...
					<span class="h5 mb-0 d-flex center-middle"><i class="far fa-list-alt mr-1"></i> Lista de NFEs </span>
						<div>
							<a href="{{route('admin.venda.create')}}" class="btn btn-azul mx-1 d-inline-block"><i class="fas fa-plus-circle"></i> Nova Venda</a>						
						  	<a href="{{route('admin.notafiscal.create')}}" class="btn btn-azul mx-1 d-inline-block"><i class="fas fa-plus-circle"></i> Gerar Nova Nota</a>
						  	<a href="{{route('admin.notafiscal.lerArquivo')}}" class="btn btn-azul mx-1 d-inline-block"><i class="fas fa-plus-circle"></i> Importar NFE</a>
            	...
            	<div class="rows">
					...
					<div class="col-2 MostraOpcoes" id="opcoes_nfe">
							<ul class="cx-opcoes">
								...
								<li class="concreto"><a href="javascript:;" onclick="imprimirDanfe()" title="Gerar PDF"><i class="fas fa-file-pdf"></i> Visualizar PDF DANFE</a></li>
                                ...
                                <li class="concreto"><a href="javascript:;" onclick="devolucaoVendaNfe()" title="Gerar Devolução"><i class="fas fa-print"></i> Gerar Devolução</a></li> 
...
function imprimirDanfe(){	
	giraGira();
	window.open(base_url + 'admin/nfe/imprimirDanfePelaNfe/'+nfe_id, '_blank');
	location.reload();	
}
...
function editarNfe(){
	giraGira();
	location.href = base_url + "admin/notafiscal/edit/" + nfe_id;	
}

function editarLivre(){
	giraGira();
	location.href = base_url + "admin/notafiscal/edicaoLivre/" + nfe_id;	
}

function devolucaoVendaNfe(){
	giraGira();
	location.href = base_url + "admin/notafiscal/devolucaoVenda/" + nfe_id;	
}
function excluirNfe(){
	giraGira();
	location.href = base_url + "admin/notafiscal/excluir/" + nfe_id;	
}
function baixarXmlNfe(){
	$("#gira_gira_opcoes").show(); 
	window.location.href= base_url + "admin/nfe/baixarXML/" + nfe_id;
	//$("#gira_gira_opcoes").hide();
}

function baixarPdfNfe(){
	$("#gira_gira_opcoes").show(); 
	window.location.href= base_url + "admin/nfe/baixarPdf/" + nfe_id;
	//$("#gira_gira_opcoes").hide();
}

function consultarNfe(){
	$("#gira_gira_opcoes").show(); 
	window.open(base_url + 'admin/nfe/consultarNfe/'+ nfe_id, '_blank');
	location.reload();
	$("#gira_gira_opcoes").hide();
}
...
function transmitirNfe(){
	...
	$.ajax({
	   url: base_url + "admin/nfe/transmitirJs/" + nfe_id,
	   type: "GET",
	   dataType: "json",
	   data:{},
...
function imprimirCce(){	
	fecharModal();
	window.open(base_url + 'admin/nfe/imprimirCce/'+nfe_id, '_blank');
	location.reload();	
}
...
function imprimirCancelamento(){	
	fecharModal();
	window.open(base_url + 'admin/nfe/imprimircancelado/'+nfe_id, '_blank');
	location.reload();	
}
...
function verXml(){
	 $.ajax({
		  url: base_url + "admin/nfe/verXML/" + nfe_id ,
		  type: "GET",
		  dataType: "json",
		  data:{},
		  success: function (data){
