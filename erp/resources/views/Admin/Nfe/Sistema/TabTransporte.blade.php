			...
			<div class="rows p-2">
				<div class="col-6 mb-3">
				   <label class="text-label">Nome da transportadora (<a href="javascript:;" onclick="abrirModal('#janela_transportadora')" class="btn btn-roxo d-inline-block btn-pequeno" style="padding: 0.25rem 0.6rem;">Buscar</a>)</label>
                    <input type="text" name="transp_xNome" id="transp_xNome" value="<?php echo  $nfetransporte->transp_xNome ?? old('transp_xNome') ?>"  class="form-campo">
				</div>
				<div class="col-3 mb-3">
                    <label class="text-label">CNPJ/CPF transport.</label>	
                    <input type="text" name="transp_CNPJ"  id="transp_CNPJ" value="<?php echo  $nfetransporte->transp_CNPJ ?? old('transp_CNPJ') ?>"  class="form-campo">
				</div>
				<div class="col-3 mb-3">
                    <label class="text-label">Inscr. Est. transp.</label>	
                    <input type="text" name="transp_IE" id="transp_IE" value="<?php echo  $nfetransporte->transp_IE ?? old('transp_IE') ?>"  class="form-campo">
				</div>
				<div class="col-6 mb-3">
                    <label class="text-label">Endereço completo</label>	
                    <input type="text" name="transp_xEnder" id="transp_xEnder" value="<?php echo $nfetransporte->transp_xEnder ?? old('transp_xEnder') ?>"  class="form-campo">
				</div>
				<div class="col-4 mb-3">
                    <label class="text-label">Município</label>	
                    <input type="text" name="transp_xMun" id="transp_xMun" value="<?php echo  $nfetransporte->transp_xMun ?? old('transp_xMun') ?>"  class="form-campo">
				</div>
				
				<div class="col-2 mb-3">
                    <label class="text-label">UF transportador</label>	
                    <input type="text" name="transp_UF" id="transp_UF" value="<?php echo  $nfetransporte->transp_UF ?? old('transp_UF') ?>"  class="form-campo">
                </div>		
			</div>
	</fieldset>
...
<legend class="h5 mb-0 text-left">Veículo/Reboque/Balsa/Vagão</legend>
			<div class="rows p-2">				
				<div class="col-4 mb-3">
                    ...
                    <input type="text" name="transp_placa" id="transp_placa" value="<?php echo  $nfetransporte->transp_placa ?? old('transp_placa') ?>"  class="form-campo">
				</div>
				<div class="col-4 mb-3">
                    <label class="text-label">UF veículo</label>	
                     <input type="text" name="UF_placa" id="UF_placa" value="<?php echo $nfetransporte->UF_placa ?? old('UF_placa') ?>"  class="form-campo">
                    
				</div>
				<div class="col-4 mb-3">
                    <label class="text-label">Reg. Nac. Trans. Carga</label>	                   
                    <input type="text" name="RNTC" id="RNTC" value="<?php echo $nfetransporte->RNTC ?? old('RNTC') ?>"  class="form-campo">
				</div>
				<div class="col-6 mb-3">
                    <label class="text-label">Identificação vagão</label>	                   
                    <input type="text" name="transp_vagao" id="transp_vagao" value="<?php echo $nfetransporte->transp_vagao ?? old('transp_vagao') ?>"  class="form-campo">
				</div>
				<div class="col-6 mb-3">
                    <label class="text-label">Identificação balsa</label>	                   
                    <input type="text" name="transp_balsa" id="transp_balsa" value="<?php echo $nfetransporte->transp_balsa ?? old('transp_balsa') ?>"  class="form-campo">
				</div>
			</div>	
</fieldset>
...
-->
			...
			<div class="rows p-2">
				<div class="col mb-1">
					...
					<input type="text" name="vol_qtde" id="vol_qtde" value="<?php echo $nfetransporte->qVol ?? old('qVol') ?>"  class="form-campo">
				</div> 
				<div class="col mb-1">
					<label class="text-label">Espécie dos vol.</label>	                   
					<input type="text" name="vol_especie" id="vol_especie" value="<?php echo $nfetransporte->esp ?? old('esp') ?>"  class="form-campo">
				</div>

				<div class="col mb-1">
					<label class="text-label">Marca dos volumes</label>	                   
					<input type="text" name="vol_marca" id="vol_marca" value="<?php echo $nfetransporte->marca ?? old('marca') ?>"  class="form-campo">
				</div>      
				<div class="col mb-1">
					<label class="text-label">Numeração dos vol.</label>	                   
					<input type="text" name="vol_numeraco" id="vol_numeraco" value="<?php echo $nfetransporte->nVol ?? old('nVol') ?>"  class="form-campo">
				</div>      
				<div class="col mb-1">
					<label class="text-label">Peso líquido</label>	                   
					<input type="text" name="vol_peso_liq" id="vol_peso_liq" value="<?php echo $nfetransporte->pesoL ?? old('pesoL') ?>"  class="form-campo">
				</div>     
				<div class="col mb-1">
					<label class="text-label">Peso bruto</label>	                   
					<input type="text" name="vol_peso_bruto"  id="vol_peso_bruto" value="<?php echo $nfetransporte->pesoB ?? old('pesoB') ?>"  class="form-campo">
				</div>      
				<div class="col mb-1">
					<label class="text-label">Números dos lacres</label>	                   
					<input type="text" name="vol_lacre" id="vol_lacre" value="<?php echo $nfetransporte->nLacre ?? old('nLacre') ?>"  class="form-campo">
				</div>           
			</div>
			</fieldset>