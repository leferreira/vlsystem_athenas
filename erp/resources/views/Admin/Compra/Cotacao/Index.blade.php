		...
		<div class="col-12">
            <div class="px-2">
                    ...
                    <table cellpadding="0" cellspacing="0" id="dataTable" width="100%">
                            ...
                            <tbody>
                       @foreach($lista as $cotacao)                                           
                             <tr>
                                <td align="center">{{$cotacao->id}}</td>
                                <td align="center">{{databr($cotacao->data_abertura)}}</td>
                                <td align="center">{{$cotacao->status->status_cotacao}}</td>
                        	@if($cotacao->status_cotacao_id == 1)                             											
                                <td align="center"> <a href="{{route('cotacao.create')}}" class="d-inline-block btn btn-outline-roxo btn-pequeno"><i class="fas fa-edit"></i> Continuar</a></td>                                   											
						    @elseif($cotacao->status_cotacao_id == 3)                            											
                                <td align="center"> <a href="{{route('cotacao.comparar',$cotacao->id)}}" class="d-inline-block btn btn-outline-roxo btn-pequeno"><i class="fas fa-edit"></i> Cotar</a></td>                                   											
                            @else                              											
                                <td align="center"> <a href="javascript:;" class="d-inline-block btn btn-outline-roxo btn-pequeno"><i class="fas fa-edit"></i> Sem Ação</a></td>                                   											
                            @endif 
                       </tr>                                       
                        @endforeach                                        
