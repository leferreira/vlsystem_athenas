	...
	<div class="corpo">
		<div class="lista">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="border">
				<thead>
					<tr> 
						<th colspan="10" class="border-bottom" style="text-transform:uppercase;padding:15px 4px;font-size:16px">RELATORIO  DE RECEBIMENTOS</th>
					</tr>
					<tr class="thead"> 
						<th align="left" class="border-bottom">ID</th> 
						<th align="left" class="border-bottom">Descrição</th> 
				...
				@foreach($lista as $v)	
					<tr style="text-transform:uppercase">
						<td class="border-bottom" align="left">{{$v->id}}</td>
						<td class="border-bottom" align="center"><p>{{$v->descricao_recebimento}}</p></td>
						<td class="border-bottom" align="left">{{databr($v->data_recebimento)}}</td>						
						<td class="border-bottom" align="center"><p>{{$v->numero_documento}}</p></td>
						<td class="border-bottom" align="center"><p>{{$v->valor_original}}</p></td>				 
						<td class="border-bottom" align="center"><p>{{$v->juros}} </p>	</td>	
						<td class="border-bottom" align="center"><p>{{$v->desconto}} </p>	</td>	
						<td class="border-bottom" align="center"><p>{{$v->multa}} </p>	</td>	
						<td class="border-bottom" align="center"><p>{{$v->valor_recebido}} </p>	</td>
						<td class="border-bottom" align="center"><p>{{ ($v->forma_pagamento) ? $v->forma_pagamento->forma_pagto : '--' }} </p>	</td>			
					</tr>
				@endforeach
