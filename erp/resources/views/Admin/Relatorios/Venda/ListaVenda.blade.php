	...
	<div class="corpo">
		<div class="lista">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="border">
				<thead>
					<tr> 
						<th colspan="7" class="border-bottom" style="text-transform:uppercase;padding:15px 4px;font-size:16px">RELATORIO  DE VENDAS: de {{databr($filtro->data1)}} a {{databr($filtro->data2)}}</th>
					</tr>
					<tr class="thead"> 
						<th align="center"  class="border-bottom">Id</th>
						<th align="center" class="border-bottom">Data</th>
						<th align="left" class="border-bottom">Cliente</th>
						<th align="center" class="border-bottom">Valor</th>
						<th align="center" class="border-bottom">Status Venda</th>
						<th align="center" class="border-bottom">Status Financeiro</th>
						<th align="center" class="border-bottom">Vendedor</th>				
					</tr>
				</thead>
				...
				@foreach($lista as $c)	
					<tr style="text-transform:uppercase">
    					<td class="border-bottom"  align="center">{{$c->id}} </td>
    					<td class="border-bottom"  align="center">{{ databr($c->data_venda)}}</td>	
    					<td class="border-bottom"  align="center">{{substr($c->cliente->nome_razao_social, 0, 30)}}</td>    									
    					<td class="border-bottom"  align="center">{{ $c->valor_venda  }}	</td>
    					<td class="border-bottom"  align="center"><span class="{{ strtolower($c->status->status) }}">{{ $c->status->status }}</span></td>									
    					<td class="border-bottom"  align="center"><span class="{{ strtolower($c->status_financeiro->status) }}">{{ $c->status_financeiro->status }}</span></td>
    					<td class="border-bottom"  align="center">{{substr($c->vendedor->nome, 0, 30)}}</td> 
