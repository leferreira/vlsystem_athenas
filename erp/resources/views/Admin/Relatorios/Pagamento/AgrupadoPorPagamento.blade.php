	...
	<div class="corpo">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="border">
				<thead>
					<tr> 
						...
						Lista de Pagamentos agrupados por dia de Pagamento<br> 
						<small style="font-weight:initial">Filtrado por data de emissão. De 01/12/2022 até 05/12/2022 com as situação</small>
						</th>
					</tr>
				</thead>
			</table>
		@foreach($lista as $l)	
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="border" style="border-top:0;margin-bottom:20px">
				<thead>
					<tr class="thead"> 
						<th align="left" colspan="10" class="border-bottom border-top" style="font-size:1rem!important">Pagamento: {{databr($l->data)}}</th> 
					</tr>
					<tr class="thead"> 
                       ...
                       <th align="center" class="border-bottom">Valor Pago</th>
				...
				@php
					$soma_total_recebido += $lancamento->valor_pago;			
					
				@endphp
					<tr style="text-transform:uppercase">						
					   <td class="border-bottom border-right" align="center">{{ $lancamento->id }}</td>
                       <td class="border-bottom border-right" align="left">{{ $lancamento->descricao_pagamento }} </td>
                       <td class="border-bottom border-right" align="center">{{ databr($lancamento->data_pagamento) }}</td>
                       ...
                       <td class="border-bottom border-right" align="center">{{ ($lancamento->forma_pagto) ? $lancamento->forma_pagto->forma_pagto : '--' }}</td> 
                       <td class="border-bottom border-right" align="center">{{ formataNumeroBr($lancamento->valor_pago) }}</td>
