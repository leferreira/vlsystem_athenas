...
class RequisitoPdvService
{
    protected $empresaRepositorio;
    protected $categoriaRepositorio;
    
    public function __construct(EmpresaRepositorioInterface $empresaRepositorio) {
        $this->empresaRepositorio = $empresaRepositorio;
        
    }        
    
    public function verificaRequisito(string $uuid){
        ...
        $natureza_operacao = NaturezaOperacao::where(["padrao"=>config('constantes.padrao_natureza.PDV'), "empresa_id" => $empresa->id])->first();
     ...
     /*       if(!$emitente->indIntermed || !$emitente->cnpjIntermed || !$emitente->idCadIntTran){
        ...
        $numCaixa = PdvCaixaNumero::where("empresa_id", $empresa->id)->first();
        if(!$numCaixa){
            $retorno->tem_erro = true;
